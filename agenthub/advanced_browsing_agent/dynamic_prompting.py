...
from agenthub.advanced_browsing_agent.llm_utils import (
    ParseError,
    count_tokens,
    extract_code_blocks,
...
class HTML(Trunkater):
    def __init__(
        self, html, visible_elements_only: bool, visible: bool = True, prefix=''
    ) -> None:
...
class Error(PromptElement):
    def __init__(
        self, error: str, visible: bool = True, prefix='', limit_logs=True
    ) -> None:
...
# {self.ax_tree.prompt}{self.focused_element.prompt}
...
"""
    ...
    def add_screenshot(self, prompt):
        if self.flags.use_screenshot:
            ...
            img_url = BrowserEnv.image_to_jpg_base64_url(screenshot)
            prompt.append(
                {
                    'type': 'image_url',
                    'image_url': {
...
class ChatInstructions(PromptElement):
    def __init__(
        self, chat_messages, visible: bool = True, extra_instructions=None
    ) -> None:
        super().__init__(visible)
        self._prompt = """\
# Instructions
...
"""
    ...
    def __init__(
        self, action_set: AbstractActionSet, action_flags: ActionFlags
    ) -> None:
        super().__init__()
    ...
    def _parse_answer(self, text_answer):
        try:
            return parse_html_tags_raise(
                text_answer, keys=['think'], merge_multiple=True
            )
        except ParseError as e:

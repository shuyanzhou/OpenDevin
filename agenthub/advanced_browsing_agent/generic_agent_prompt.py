...
from agenthub.advanced_browsing_agent.llm_utils import parse_html_tags_raise
from opendevin.events.observation import Observation as ODObservation
...
class MainPrompt(dp.Shrinkable):
    def __init__(
        ...
        goal: str,
        obs_history: list[ODObservation],
        actions: list[str],
        memories: list[str],
        thoughts: list[str],
        previous_plan: str,
        ...
        if self.flags.enable_chat:
            raise NotImplementedError('Chat mode for web agent is not yet implemented.')
            ...
            #         [
            #             msg["role"] == "user"
            #             for msg in obs_history[-1].get("chat_messages", [])
            #         ]
            #     )
            #     > 1
            # ):
            #     logging.warning(
            #         "Agent is in goal mode, "
            #         "but multiple user messages are present in the chat."
        ...
        self.think = dp.Think(visible=flags.use_thinking)
        self.hints = dp.Hints(visible=flags.use_hints)
        self.plan = Plan(previous_plan, step, flags.use_plan)  # TODO add previous plan
        self.criticise = Criticise(visible=flags.use_criticise)
...
"""
    ...
    def _parse_answer(self, text_answer):
        return parse_html_tags_raise(
            text_answer, optional_keys=['action_draft', 'criticise']
        )
